...
def handle_exception (exception: CustomError) :
    if exception.error_type == ErrorType.FILE_NOT_FOUND:
        print(f"Error: File was not found ! {exception.message}")
    elif exception.error_type == ErrorType.DIRECTORY_NOT_FOUND:
        print(f"Error: Directory was not found ! {exception.message}")
